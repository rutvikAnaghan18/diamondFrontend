...
@Component({
  selector: 'app-upload-base-doc',
  templateUrl: './upload-base-doc.component.html',
  styleUrls: ['./upload-base-doc.component.css']
})
export class UploadBaseDocComponent implements OnInit {
  ...
  uploadFile(event: any) {
    ...
    fileReader.onload = (e) => {
      ...
      this.arrayList.map((item) => {
        item.tenderName = this.file.name
        item.basePricePerCT = Number((item.basePrice/item.weight).toFixed(2))
      })
    };
  ...
  submit(){
    if (this.arrayList) {
      this.diamondService.uploadBaseFile(this.arrayList).subscribe((res) => {
